...
func Test_Receive_GitRepository(t *testing.T) {
	...
	notifications, err := env.Store.GetByObject("GitRepository", "flux-system", "podinfo", "v1.8.0/6c8a85a5ab953874c7c83d50317359a0e5a352a9")
	require.NoError(t, err)
...
func Test_Receive_Kustomization(t *testing.T) {
	...
	notifications, err := env.Store.GetByObject("Kustomization", "flux-system", "podinfo", "v1.8.0/6c8a85a5ab953874c7c83d50317359a0e5a352a9")
	require.NoError(t, err)
	require.Len(t, notifications, 1)
...
func Test_Receive_MultipleEventsDifferentType(t *testing.T) {
	rr := httptest.NewRecorder()
	env := handlers.Env{receiver.NewInMemoryStore()}
	handler := http.Handler(handlers.Handler{Env: env, H: handlers.ReceiveNotification})
	...
	notifications, err := env.Store.GetByRevision("v1.8.0/6c8a85a5ab953874c7c83d50317359a0e5a352a9")
	require.NoError(t, err)
	require.Len(t, notifications, 2)
	assert.Equal(t, notifications[0], GitRepositoryEvent)
	assert.Equal(t, notifications[1], KustomizationEvent)
}
